...
class GetPopularMoviesStrategyTest {
    ...
    @Before
    fun setup() {
        getProgramsStrategy = GetPopularMoviesStrategy(movieRepository)
    }

    @Test
    fun `when execute is called then should call getPopularMovies and return its value`() {
        val flowPrograms = flowOf(mockk<List<Program>>())
        every { movieRepository.getPopularMovies() } answers { flowPrograms }
